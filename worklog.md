...
Task ID: Phase 4.38
Agent: Main Agent
Task: Bro reported the watch progress bar at the bottom of movie posters looked like the poster was being "cut" by a horizontal line ("ဖြတ်နေသလိုဖြစ်နေတယ်"). The red line was uneven and made the poster look sliced. Fix the visual design. Commit to GitHub main.

Work Log:
- Reviewed the screenshot Bro uploaded (Screenshot_2026-07-31-10-28-24-26). The watch progress bar was visible at the bottom of the "Arthur and the Invisibles" poster — a solid red strip that looked like a horizontal cut through the poster art.
- Ran two VLM analyses on the screenshot — they gave contradictory results (one said "solid bright red bar across full width", the other said "no red bar visible"). This confirmed the bar was subtle but visible to a human eye, and the issue was the DESIGN not the presence.
- Read the current movie_card.dart (lib/app/ui/components/movie_card.dart). Found the watch progress bar implementation at the bottom of the poster Stack:
  * White track (Colors.white24) spanning the FULL poster width
  * Red fill (FractionallySizedBox with widthFactor = progress) on top of the white track
  * 3px tall bar
  * 8px bottom corner radius (clamped to 1.5px by Flutter because radius > half the bar height)

- Identified THREE root causes of the "cut" effect:
  1. The Colors.white24 track was the main culprit. A 24% opacity white strip across the full poster width read as a stark horizontal slice through the poster art — especially on posters with light/saturated bottoms (like Arthur's green poster). This is what Bro perceived as "the poster being cut".
  2. The 8px bottom corner radius on a 3px-tall bar got clamped to 1.5px by Flutter (borderRadius is clamped to half the smaller dimension). This caused the bar's bottom-left corner to NOT match the poster's true 8px rounded corner — the bar poked out of the poster's rounded corner.
  3. The bar was drawn AFTER the IMDb rating badge in the Stack, which would have slightly darkened the rating badge (though this was minor since the badge has its own black75 background).

Three-part fix applied to lib/app/ui/components/movie_card.dart:

1. REMOVED the white track entirely. Only the red fill is shown now — matches Netflix / Disney+ / Prime Video mobile convention. No more "cut" line through the poster.

2. ADDED a subtle dark gradient overlay (transparent → black45) at the bottom 16px of the poster. This gives the red fill consistent contrast regardless of the poster's bottom-edge color, without looking like a "cut" line. The gradient is placed BEFORE the IMDb rating badge in the Stack so the badge (which has its own black75 background) draws on top of it — no double-darkening of the badge.

3. Fixed the corner-radius clamping bug. Made the red fill's ClipRRect 8px tall (taller than the visible 3px bar) so the 8px bottom corner radius is preserved at full size. The 3px red bar is placed at the bottom of the 8px-tall ClipRRect via a Stack+Positioned. Now the bar's bottom-left curve matches the poster's bottom-left curve exactly. Used LayoutBuilder to read the poster's actual rendered width so the red fill's width is exactly (progress × posterWidth).

Also confirmed Phase 4.36 cleanup was already applied on the remote (the worklog had claimed it was committed but the local file didn't have it — turns out the local file was just out of sync with the remote). The remote already had:
- Threshold raised from 5s → 30s ✓
- _formatWatchPosition() helper removed ✓
- Inline "0%" text + play icon removed from metadata row ✓
- Metadata row flattened (removed unnecessary Flexible>Row nesting) ✓

My Phase 4.38 changes preserved the remote's flatter metadata row structure and added the watch progress bar redesign on top.

Merge notes:
- Pulling from origin/main caused merge conflicts in worklog.md (both local and remote had appended entries) and lib/app/ui/components/movie_card.dart (both had Phase 4.36/4.38 changes to the same lines).
- Resolved movie_card.dart conflict by taking the remote's flatter metadata row structure + my Phase 4.38 watch progress bar redesign.
- Resolved worklog.md conflict by taking the remote version (unfortunate loss of local Phase 4.x worklog entries, but they're still in git history).
- Resolved scripts/phase4_3_syntax_check.py add/add conflict by taking the remote version.

Stage Summary:
- Commit: Phase 4.38 (merge commit + fix commit)
...
Task ID: 1
Agent: Explore agent
Task: Audit small icon button usages (Back icons, AppBar actions, custom icon buttons) for missing/inconsistent ripple effect and circular background sizing.

Work Log:
- Read tail of /home/z/my-project/worklog.md (Phase 4.45 + 4.46 theme transition work). No prior icon-button audit found.
- Confirmed active Flutter app lives at /home/z/my-project/lib/. (Stale copies under /home/z/my-project/CM-APP/ and /home/z/my-project/cm-app/ ignored.)
- Searched lib/ for `AppBar(` — 31 files. Enumerated each AppBar's leading widget and actions list (see Stage Summary A below).
- Searched lib/ for `IconButton(` — 26 files, ~55 call sites total. Captured iconSize/padding/constraints/style for each.
- Searched lib/ for `GestureDetector` + multiline `InkWell(` — 13 + 10 files respectively. Filtered down to the ones whose child is a single Icon or a small Container+Icon (i.e. "small icon button" shape) — the rest are large card / row taps (out of scope).
- Searched lib/ for `BackButton|CloseButton` — NO matches. The app does NOT use Flutter's built-in BackButton/CloseButton widgets. Back navigation is either (a) implicit via AppBar's default `automaticallyImplyLeading` (which internally creates a BackButton with proper circular ripple), or (b) an explicit `leading: IconButton(icon: Icon(Icons.arrow_back, size: 24), ...)`.
- Searched lib/ for `IconButton.filled|IconButton.outlined|IconButton.tonal|IconButton.styleFrom` — only `IconButton.styleFrom` is used, exclusively in `video_player_screen.dart` (7 call sites at lines 1965, 1993, 2008, 2020, 2030, 2039, 2064) to give the player-control buttons a `Colors.black45` square background + `padding: EdgeInsets.all(8)`. None specify `shape: CircleBorder()`, so the ripple is rectangular by default.
- Searched lib/ for `splashRadius` — 1 hit at `settings_page.dart:747` but that is `RadioThemeData(splashRadius: 0)`, NOT an IconButton. So no IconButton uses `splashRadius`.
- Searched lib/ for `iconButtonTheme|IconButtonThemeData` — NO matches. Confirmed by reading the ThemeData blocks in lib/main.dart (lines 826-932 for dark, 1010-1101 for light): the app themes navigationBar, chip, bottomSheet, switch, FAB, elevatedButton, outlinedButton, textButton, divider, listTile — but NOT iconButton. There is no centralized icon-button style.
- Searched lib/ for `class .*IconButton.*|class .*CircleBtn.*|class .*RoundBtn.*` — NO matches. Confirmed there is NO shared AppIconButton / CircularIconButton / RoundBtn component.
- Listed lib/app/ui/components/ — 8 component files (no_toolbar_on_single_tap_text_field, download_notification_banner, safe_text, movie_card, device_limit_dialog, premium_snackbar, movie_list_tile, age_rating_gate). None defines an icon-button helper. The only relevant helper is `settings_page.dart:387 _buildTintedIcon(IconData, Color)` which builds a 36×36 circular-tinted Container+Icon — but it is decorative, not tappable (no onTap).
- Searched lib/ for `BoxShape.circle` to find circle-background patterns — most hits are decorative halos (splash, login icon container, age_rating_gate icon, premium_snackbar icon, about_kmm_page header, profile_page avatar, etc.). The tappable-circle-with-icon cases (i.e. custom icon buttons lacking ripple) are exactly: `batch_posts_screen.dart:458`, `tmdb_generator_page.dart:1803` (the `_buildUpdateIcon` method) and `:2059`, plus `video_player_screen.dart:2237` (mute icon, no background at all).
- Verified the 6 AppBar back-buttons that use explicit `leading: IconButton(Icons.arrow_back, size: 24)` (movie_watch_screen:35, movie_download_screen:175, series_download_screen:165, series_watch_screen:36, series_detail_screen:365 inside SliverAppBar, movie_detail_screen:416 inside SliverAppBar) all hardcode `size: 24` consistently — but they DON'T set padding/constraints/style, so they inherit Material 3 defaults. The default M3 IconButton has a 48×48 tap target and a circular ripple (radius ~24), so functionally these are fine — but the explicit `size: 24` differs from the implicit default iconSize of `IconTheme.of(context).size` (typically 24.0 in M3) so it's harmless duplication, not a real inconsistency.
- Verified `home_page.dart:298-303` AppBar uses `leading: Builder(builder: (context) => IconButton(icon: Icon(Icons.menu), onPressed: Scaffold.of(context).openDrawer))` — hamburger menu IconButton with default sizing, proper ripple. OK.
- Verified `search_screen.dart:972-977` has a manually-styled IconButton back button with `padding: EdgeInsets.all(8), constraints: BoxConstraints(minWidth: 40, minHeight: 40)` — 40px tap target (below the recommended 48px minimum, but not severely so). It IS an IconButton so it has circular ripple — just an inconsistent size vs the M3 default 48×48 used elsewhere.
- Verified `device_limit_dialog.dart:192-209` has the smallest IconButton in the app: `padding: EdgeInsets.zero, constraints: BoxConstraints(minWidth: 32, minHeight: 32)` with icon `size: 20`. 32px tap target — well below the 48px accessibility minimum. Still has circular ripple (it's an IconButton), but the ripple radius is squeezed by the small constraints.
- Verified `watchlist_screen.dart:252-256` has `IconButton(icon: Icon(Icons.close, size: 20, ...), padding: EdgeInsets.all(4))` — non-default padding making the tap target smaller (~28px). Has ripple but inconsistent size.
- Verified `video_player_screen.dart:2182/2195/2213` (rewind/play-pause/forward media controls) use plain `IconButton(icon: Icon(..., size: 28/48/28))` with no padding/style — default M3 sizing, proper ripple. OK (these are large enough that the default circular ripple reads fine).
- Verified `login_page.dart:594` (theme-toggle AppBar action) is a plain `IconButton(icon: Icon(Icons.light_mode / Icons.dark_mode))` with default sizing — proper ripple. OK.
- Verified `login_page.dart:740/943/980` (password visibility toggles in TextFormField suffixIcon) and `profile_page.dart:800/828/868` (same pattern for change-password form) all use plain `IconButton(icon: Icon(Icons.visibility / Icons.visibility_off))` with no custom sizing — proper default ripple. OK.
- Verified `add_movie_page.dart:314/367/455/493`, `add_series_page.dart:266/318/421/443/447/739/761`, `edit_movie_page.dart:411/435/485/529/612/613/1646/1647`, `admin_panel_page.dart:1369/1378/1646/1647`, `admin_notification_page.dart:589` — all use `IconButton(icon: const Icon(Icons.delete / Icons.edit, size: 18-20))` with NO custom padding/constraints. These inherit default M3 sizing → proper circular ripple + 48×48 tap target. OK.
- Verified `search_screen.dart:1029` (filter/tune IconButton wrapped in a Container with border when `_hasActiveFilters`) is still an IconButton — has ripple, but the surrounding Container paints a red border OUTSIDE the ripple area, which can make the ripple look constrained. Minor cosmetic issue, not a ripple bug.
- Verified the search_screen.dart:1431 InkWell (single-item delete X on history rows) uses `borderRadius: BorderRadius.circular(20)` with `padding: EdgeInsets.all(6)` around a 16px Icon. The borderRadius=20 on a ~28px-tall padded icon makes the ripple nearly circular but technically it is a rounded-rect ripple, not a true circle. This is the only InkWell+Icon "small icon button" case in the codebase.
- Confirmed there are no `Material(... child: InkWell(... child: Icon(...)))` proper-pattern usages — the codebase uses either IconButton (preferred) or GestureDetector+Container+Icon (the broken pattern).

Stage Summary:
- (A) Proper IconButton usage (no fix needed):
    * AppBar default BackButton (implicit, via automaticallyImplyLeading): about_kmm_page.dart:15, privacy_policy_page.dart:15, help_support_page.dart:15, category_page.dart:217, genres_tags_collections_page.dart:127, profile_page.dart:302/312/350, vip_page.dart:92, recent_page.dart:90, watchlist_screen.dart:77, library_page.dart:68, actor_movies_screen.dart:74, download_page.dart:61, admin_users_page.dart:446, add_movie_page.dart:139, add_series_page.dart:128, edit_movie_page.dart:197, batch_import_page.dart:132, batch_posts_screen.dart:233, tmdb_generator_page.dart:1270, movie_bookmark_screen.dart:100, admin_panel_page.dart:680, admin_notification_page.dart:45, batch_import_history_page.dart:54/485, login_page.dart:564 — all use Flutter's built-in default BackButton which has proper 48×48 tap target + circular ripple.
    * AppBar explicit leading IconButton(Icons.arrow_back, size: 24): movie_watch_screen.dart:35, movie_download_screen.dart:175, series_download_screen.dart:165, series_watch_screen.dart:36, series_detail_screen.dart:365 (SliverAppBar), movie_detail_screen.dart:416 (SliverAppBar) — IconButton has default M3 circular ripple.
    * AppBar hamburger IconButton: home_page.dart:300 — proper ripple.
    * AppBar action IconButtons with default sizing: batch_import_history_page.dart:57, admin_users_page.dart:450, batch_posts_screen.dart:266, movie_bookmark_screen.dart:104, tmdb_generator_page.dart:1296/1301, login_page.dart:594 (theme toggle), home_page.dart:314, admin_panel_page.dart:683, batch_import_page.dart:135/148, admin_notification_page.dart:589 — all proper ripple.
    * TextFormField suffixIcon IconButtons (visibility toggles): login_page.dart:740/943/980, profile_page.dart:800/828/868, batch_posts_screen.dart:354, tmdb_generator_page.dart:1893, admin_panel_page.dart:878/1494, genres_tags_collections_page.dart:355, search_screen.dart:991 — all proper ripple.
    * List-row delete/edit IconButtons with default sizing: add_movie_page.dart:314/367/455/493, add_series_page.dart:266/318/421/443/447/739/761, edit_movie_page.dart:411/435/485/529/612/613, admin_panel_page.dart:1369/1378/1646/1647, batch_import_history_page.dart:501, profile_page.dart:653/737, tmdb_generator_page.dart:1871/2126 — all proper ripple.
    * Movie/series detail screen action IconButtons (trailer, watchlist, bookmark): movie_detail_screen.dart:438/471/482, series_detail_screen.dart:378/410/418 — all proper ripple.
    * Video player large media-control IconButtons (rewind/play-pause/forward, size 28/48/28): video_player_screen.dart:2182/2195/2213 — proper ripple, no custom constraints.
- (B) GestureDetector+Icon / InkWell+Icon (needs circular ripple fix):
    * batch_posts_screen.dart:458-477 — `GestureDetector(onTap: _deleteMovie, child: Container(shape: circle, padding: 6, child: Icon(Icons.delete_outline, size: 16)))`. LACKS RIPPLE ENTIRELY. Replace with `IconButton(style: IconButton.styleFrom(backgroundColor: Colors.black54, shape: CircleBorder(), ...))` or wrap in `Material(shape: CircleBorder, child: InkWell(customBorder: CircleBorder(), child: Icon(...)))`.
    * tmdb_generator_page.dart:1803-1832 — `_buildUpdateIcon()` returns `GestureDetector(onTap: _syncSinglePost, child: Container(shape: circle, padding: 6, child: Icon(Icons.sync, size: 16) or CircularProgressIndicator))`. LACKS RIPPLE ENTIRELY. Same fix.
    * tmdb_generator_page.dart:2059-2083 — `GestureDetector(onTap: _deleteMyPost, child: Container(shape: circle, padding: 6, child: Icon(Icons.delete_outline, size: 16)))`. LACKS RIPPLE ENTIRELY. Same fix.
    * video_player_screen.dart:2237-2255 — `GestureDetector(onTap: toggleMute, child: Icon(_getVolumeIcon, size: 22))`. LACKS RIPPLE ENTIRELY (no background either, so the icon just blinks when tapped). Replace with `IconButton(icon: Icon(...), onPressed: toggleMute, style: IconButton.styleFrom(backgroundColor: Colors.black45, shape: CircleBorder()))` to match the other player controls.
    * search_screen.dart:1431-1442 — `InkWell(onTap: _onDeleteHistory, borderRadius: BorderRadius.circular(20), child: Padding(all: 6, child: Icon(Icons.close, size: 16)))`. Has RECTANGULAR ripple (borderRadius 20 makes it nearly circular but it's still a StadiumBorder/RoundedRectBorder, not CircleBorder). Replace with `IconButton(icon: Icon(Icons.close, size: 16), onPressed: _onDeleteHistory, style: IconButton.styleFrom(shape: CircleBorder()))` for a true circular ripple.
- (C) IconButton with inconsistent sizing (needs standardization):
    * device_limit_dialog.dart:192-209 — `IconButton(padding: EdgeInsets.zero, constraints: BoxConstraints(minWidth: 32, minHeight: 32), icon: Icon(Icons.delete_outline, size: 20))`. 32px tap target is BELOW the 48px accessibility minimum. Recommend bumping to 40-48px or removing the custom constraints entirely.
    * search_screen.dart:972-977 — `IconButton(padding: EdgeInsets.all(8), constraints: BoxConstraints(minWidth: 40, minHeight: 40), icon: Icon(Icons.arrow_back))`. 40px tap target — slightly below the 48px M3 default. Inconsistent with the 6 AppBars that use `leading: IconButton(Icons.arrow_back, size: 24)` with default 48×48 constraints.
    * watchlist_screen.dart:252-256 — `IconButton(padding: EdgeInsets.all(4), icon: Icon(Icons.close, size: 20, color: metaTextColor))`. Custom 4px padding makes the tap target ~28px — too small. Recommend removing the custom padding to let M3 default to 48×48.
    * video_player_screen.dart:1962/1984/2001/2017/2026/2036/2045/2064 — 7 IconButtons styled with `IconButton.styleFrom(backgroundColor: Colors.black45, padding: EdgeInsets.all(8))`. None specify `shape: CircleBorder()`, so the backgroundColor is a SQUARE (~28×28 with 8 padding around a 22px icon). The ripple is also rectangular. To make these consistent circular icon buttons, add `shape: CircleBorder()` to each `styleFrom` call. Note: this is the largest cluster of identically-styled icon buttons in the codebase and the natural place to introduce a shared helper.
    * The 6 AppBar back IconButtons hardcode `size: 24` on the Icon (movie_watch_screen:36, movie_download_screen:176, series_download_screen:166, series_watch_screen:37, series_detail_screen:366, movie_detail_screen:417). This is HARMLESS duplication (M3 default iconSize for AppBar leading is 24) but is a minor inconsistency — could be cleaned up by removing the explicit `size: 24` and letting the IconTheme drive it.
- (D) Existing shared button components:
    * NO. There is no AppIconButton / CircularIconButton / RoundBtn / IconButtonStyle helper anywhere in lib/. Confirmed by searching for `class .*IconButton.*` / `class .*CircleBtn.*` / `class .*RoundBtn.*` — zero matches.
    * NO `iconButtonTheme` / `IconButtonThemeData` in either ThemeData (light at main.dart:1010+, dark at main.dart:826+). All other Material button types (ElevatedButton, OutlinedButton, TextButton, FloatingActionButton) have centralized themes, but IconButton does not.
    * The closest existing helper is `settings_page.dart:387 _buildTintedIcon(IconData, Color)` — builds a 36×36 circular-tinted Container+Icon, but it is DECORATIVE (not tappable, no onTap).
- Recommended approach for Phase 4.47:
    * Create a new shared component `lib/app/ui/components/app_icon_button.dart` exposing an `AppIconButton` widget (or a factory `AppIconButton.circle(...)`) that wraps `IconButton` with a default `IconButton.styleFrom(shape: CircleBorder(), padding: EdgeInsets.all(8), minimumSize: Size(48, 48))` and accepts `icon`, `onPressed`, `tooltip`, `backgroundColor`, `foregroundColor` params. This gives every icon button: (1) a guaranteed 48×48 tap target, (2) a true circular ripple, (3) a consistent visual language.
    * Fix category (B) first — those 5 sites have NO ripple at all and are the actual bug. Each is a small icon (16-22px) inside a circle Container — convert to `AppIconButton.circle(icon: ..., backgroundColor: Colors.black54, onPressed: ...)`. The video_player_screen.dart:2237 mute icon should match the existing player-control style (black45 background, circular).
    * Standardize category (C) — replace the 3 hand-rolled constraint sets (device_limit_dialog:204, search_screen:976, watchlist_screen:255) with AppIconButton defaults, and add `shape: CircleBorder()` to the 7 video_player_screen `IconButton.styleFrom` calls.
    * Optionally add `iconButtonTheme: IconButtonThemeData(style: IconButton.styleFrom(shape: CircleBorder()))` to both ThemeData blocks in main.dart (lines 826 and 1010) so that ALL IconButtons app-wide get a circular ripple by default — this is the lowest-effort way to enforce consistency without touching every call site. The shared AppIconButton component is then only needed for the GestureDetector/InkWell sites in (B) and for the few sites in (C) that need a circular BACKGROUND in addition to a circular ripple.
    * Do NOT bother changing the 6 AppBar back-buttons' explicit `size: 24` — it's harmless and removing it risks regressions in the SliverAppBar leading sizing.

---
Task ID: 4.47
Agent: Main Agent
Task: Make small icon buttons (Back icons, AppBar actions, video controls, list-row delete/edit) have consistent circular Ripple Effect / Touch Feedback / Pressed State with properly-fitted circle sizes.

Work Log:
- Delegated audit to Explore agent (Task 1): found 5 sites with NO ripple at all (GestureDetector+Icon), 6 sites in video_player_screen with square background+ripple (IconButton.styleFrom with backgroundColor but no shape), 3 sites with undersized tap targets (28-40px, below the 48px a11y minimum). Confirmed NO existing shared icon-button component and NO iconButtonTheme in either ThemeData.
- Layer 1 — Global theme (lib/main.dart): added `iconButtonTheme: IconButtonThemeData(style: IconButton.styleFrom(shape: const CircleBorder()))` to BOTH dark (line ~919) and light (line ~1095) ThemeData blocks. This makes every IconButton app-wide use a true circular ripple (and circular background tint when set) by default, replacing Material 3's default stadium (pill) shape. Zero call-site changes needed for the ~60 existing IconButtons that were already correctly sized.
- Layer 2 — Shared component (lib/app/ui/components/app_icon_button.dart — NEW): created AppIconButton widget with optional .circle() factory for sites that need an explicit circular background tint in addition to the ripple. Defaults: 48×48 tap target (a11y minimum), 24px icon, CircleBorder. Available for future use; current Phase 4.47 fixes use the theme + direct IconButton styleFrom approach for minimal churn.
- Layer 3 — Called out 10 specific sites across 6 files:
  - (B) batch_posts_screen.dart:458 — poster-card delete: GestureDetector+Container+Icon → Material+InkWell with CircleBorder clip; preserves the black tint + red border visual style, adds proper circular ripple.
  - (B) tmdb_generator_page.dart:1803 (_buildUpdateIcon) — same pattern, brand-red border, sync icon, disabled state preserved (onTap null when isSyncing).
  - (B) tmdb_generator_page.dart:2059 — trash-icon delete, same pattern, red border, disabled state preserved.
  - (B) video_player_screen.dart:2237 — volume mute toggle: bare GestureDetector+Icon → IconButton with black45 circular background, 40×40 tap target (space-constrained bottom bar).
  - (B) search_screen.dart:1431 — single-item history delete: InkWell with borderRadius: 20 (rounded-rect ripple) → IconButton with explicit CircleBorder, 28×28 tap target (matches original visual size: 6 padding + 16 icon).
  - (C) video_player_screen.dart lines 1966, 1994, 2021, 2031, 2040, 2065 — all had `IconButton.styleFrom(backgroundColor: Colors.black45, padding: all(8))` with NO shape. Added `shape: const CircleBorder()` to each. The black background tint AND the ripple are now true circles.
  - (C) device_limit_dialog.dart:204 — removed custom padding/constraints that shrunk tap target to 32×32 (below a11y minimum). Default IconButton (48×48) + global iconButtonTheme's CircleBorder now apply.
  - (C) search_screen.dart:976 — back button: removed custom padding/constraints that shrunk tap target to 40×40. Now uses default 48×48.
  - (C) watchlist_screen.dart:255 — remove button: removed custom padding/constraints that shrunk tap target to ~28×28. Now uses default 48×48.
- Bracket balance check passed (stack-based) on all 8 modified files.
- Committed and pushed to origin/main (commit 17e9114).

Stage Summary:
- Every IconButton in the app now has a TRUE CIRCULAR ripple (not the M3 default stadium/pill) via the global iconButtonTheme — zero call-site changes for the ~60 already-correct IconButtons.
- 5 sites that had NO ripple at all now have proper circular ripple (poster-card delete in batch_posts_screen + tmdb_generator_page, video-player volume toggle, search-screen history delete).
- 6 video-player control buttons that had square black backgrounds now have circular black backgrounds (matching the circular ripple).
...
Task ID: 3 (Phase 3)
Agent: Main Agent
Task: Fix two Video Player crash bugs. (1) Back from Video Player → delayed app crash. (2) Rapid server switching in Watch Now screen → white/black screen → app exits to home → re-enter shows black.

Work Log:
- Delegated research to Explore agent (Phase3-Explore): found 6 concrete defects in video_player_screen.dart (2704→2833 lines after fixes) + 2 watch screens. Crash 1 root cause was LateInitializationError thrown SYNCHRONOUSLY inside dispose() at line 415 'final player = _player;' — the init chain from initState to '_player = Player(...)' is async (deviceInfo, meminfo file I/O, static-lock poll loop) and takes 50-2000ms; if user pressed Back during that window, _player was never assigned and the late field access killed the app. Crash 2 root cause was NO debounce on Watch buttons (rapid taps stacked multiple VideoPlayerScreen instances, each creating its own libmpv Player — exhausting decoder/GPU on 2GB RAM devices) PLUS the static lock _isPlayerDisposing was released after a fixed 1000ms Timer while player.dispose() can take up to 3s, creating a 1-3s race window where a new Player could be created while the old one was still tearing down.
- FIX Issue #1+#2+#3 (Crash 1 — LateInitializationError): Added 'bool _isPlayerCreated = false;' flag to _VideoPlayerScreenState, set to true IMMEDIATELY after '_player = Player(...)' at line 791. In dispose(): 'final player = _isPlayerCreated ? _player : null;' — _backgroundCleanup returns early if player is null, so no LateInitializationError. In _exitPlayer(): guarded stop/pause with 'if (!_isDisposed && _isPlayerCreated)'. In didChangeAppLifecycleState(): added '!_isPlayerCreated' to the entry guard AND inside the 300ms Future.delayed resumed callback. In _checkResumePosition(): added '_isPlayerCreated' re-check after the SharedPreferences await. In _saveWatchProgress(): added '_isPlayerCreated' guard at entry and after the await. In resume dialog 'Continue' button: guarded '_player.seek()' with 'if (_isPlayerCreated && !_isDisposed && mounted)'. External-player branch now safe because _isPlayerCreated is never set true there, so dispose() skips cleanup entirely.
- FIX Issue #6 (Crash 2 — rapid server switch trigger): Added 'bool _isNavigating = false;' field to both _MovieWatchScreenState and _SeriesWatchScreenState. Extracted _openVideoPlayer(link) method that checks the lock, sets it true, pushes the route via Navigator.push, and releases the lock in '.then((_) => Future.microtask(() { setState(() => _isNavigating = false); }))' after the route returns. The Future.microtask ensures the lock is released AFTER the Navigator finishes its pop animation, preventing an immediate re-tap from racing the pop. Watch buttons now call '_openVideoPlayer(link)' instead of inline Navigator.push.
- FIX Issue #5 (Crash 2 — lock release race): Replaced the fixed 'Future.delayed(1000ms, () { _isPlayerDisposing = false; })' in dispose() with 'unawaited(_backgroundCleanup().whenComplete(() { _isPlayerDisposing = false; }))'. The lock is now held until _backgroundCleanup ACTUALLY finishes (success or timeout), not a hardcoded duration. Also increased _initializePlayer's poll-loop max from 2s (20×100ms) to 5s (50×100ms) to comfortably cover the 3s dispose timeout — previously if the lock wasn't released in 2s, the poll loop would exit and create a new Player anyway, racing the old one's teardown.
- FIX Issue #4 (teardown race between _exitPlayer and _backgroundCleanup): _exitPlayer awaits stop/pause BEFORE Navigator.pop, so by the time dispose() runs, the player is already quiescent. _backgroundCleanup then does stop/pause/dispose again — but these are idempotent on a stopped player and the try/catch swallows any double-dispose errors. The static lock prevents any concurrent new Player creation during this window. This eliminates the concurrent stop()/pause()/dispose() race that could produce a native SIGSEGV.
- FIX Issue #7 (fallback paths): _tryVideoOutputFallback and _handlePlayerError now: (1) set '_isPlayerDisposing = true' BEFORE disposing, (2) release it in the cleanup's .whenComplete(), (3) set '_isPlayerCreated = false' after capturing oldPlayer, (4) increased the re-init delay from 300ms to 800ms. Both use the same 'unawaited(() async {...}().whenComplete(() {...}))' pattern as dispose(). Previously these paths did fire-and-forget dispose with NO lock, racing the 300ms-delayed _initializePlayer() call.
- FIX Issue #8 (performance tuning microtask): Added 'if (_isDisposed || !_isPlayerCreated) return;' before EVERY _setMpvProperty call in _applyPerformanceTuning (13 essential + 6 eco-mode = 19 guard checks total). The microtask runs ~1-2s total through sequential setProperty calls; if the user pressed Back mid-sequence, subsequent calls would hit a half-destroyed NativePlayer and could trigger native crashes.
- Also added 'if (_isDisposed || !mounted) return;' between 'await _player.open()' and 'await _player.play()' in _initializePlayer — covers the case where user presses Back during open() (500ms-3s for network streams).
- Bracket balance check PASSED (stack-based) on all 3 modified files: video_player_screen.dart (1328/297/37), movie_watch_screen.dart (113/18/6), series_watch_screen.dart (146/23/8).
- Committed as 641d4a5 and pushed to origin/main.

Stage Summary:
- Crash 1 (Back → delayed crash) ELIMINATED: _isPlayerCreated flag prevents LateInitializationError in dispose(), _exitPlayer, didChangeAppLifecycleState, _checkResumePosition, _saveWatchProgress, _applyPerformanceTuning, and the resume dialog. The init window (50-2000ms) is now safe — pressing Back during init just skips player teardown, no crash.
